This dataset is based on the ISBI2013 phantom; however, due to known issues with the diffusion-weighted magnetic resonance iamging (DW-MRI) signal generation of the original simulator, the DW-MRI data cannot be used to quantify tractography. Hence, we created this phantom which is **based only on the actual geometry of the bundles** of the ISBI2013 phantom. Here is a description of each file.

- `ground_truth_streamlines.tck`: the streamlines in this tractogram was created directly from the geometry of the phantom, i.e., from the geometrical description of each of the 27 bundles, and not using any given tracking algorithm.

- `ground_truth_density.nii.gz`: this is the streamline density map, which is assumed to be proportioal to the actual axonal density of each bundle. The file `ground_truth_density.scheme` is needed to fit any tractogram to the density map using COMMIT.
...
To ground-truth fODF were estimated by first computing the TODI of the ground-truth streamlines, i.e.:

```bash
tckmap ground_truth_streamlines.tck ground_truth_fODF.nii.gz -template ground_truth_density.nii.gz -tod 8 -precise
```

and then normalizing them to obtain probability density functions, i.e., summing up to 1:
